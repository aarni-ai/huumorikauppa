...
import { findDialect, dialects } from "@/data/dialects";
import { situationGifts } from "@/data/situationGifts";

const DialectPage = () => {
  const { slug } = useParams<{ slug: string }>();
  const dialect = slug ? findDialect(slug) : undefined;
  ...
  const relatedDialects = dialects
    .filter((d) => dialect.relatedSlugs.includes(d.slug))
    .slice(0, 4);

  const popularSituations = situationGifts
    .filter((s) => ["tyokaverille", "syntymapaivasankarille"].includes(s.slug))
    .slice(0, 4);

  return (
    <div className="min-h-screen">
      <SEOHead
        title={dialect.seoTitle}
        description={dialect.seoDescription}
        canonical={canonical}
      ...
      <div className="container py-8 md:py-12">
        <nav aria-label="Murupolku" className="text-sm text-muted-foreground mb-6">
          ...
          <span className="mx-2">/</span>
          <span className="text-foreground">{dialect.emoji} {dialect.h1}</span>
        </nav>

        <h1 className="font-display text-3xl md:text-4xl text-foreground mb-2">
          {dialect.emoji} {dialect.h1}
        </h1>
        ...
        <div className="bg-primary/5 border border-primary/20 rounded-lg px-5 py-4 mb-6 max-w-2xl">
          ...
          <p className="text-sm text-foreground leading-relaxed">{dialect.tldr}</p>
        </div>

        <p className="text-muted-foreground mb-6 max-w-3xl">{dialect.intro}</p>
        <p className="text-sm text-muted-foreground mb-8">Suosituimmat tuotteet {dialect.region}n tyyliin</p>
        ...
        <section className="mt-10 p-6 bg-muted/30 rounded-xl border border-border">
          <h2 className="font-display text-xl text-foreground mb-3">
            {dialect.name} — tunnistettavimmat ilmaisut
          </h2>
          <div className="space-y-3">
            {dialect.expressions.map((expr, i) => (
              <div key={i} className="flex items-start gap-3">
                <span className="text-primary font-semibold min-w-[120px]">"{expr.expression}"</span>
                <span className="text-muted-foreground text-sm">= {expr.meaning}</span>
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="mt-6 p-6 bg-muted/30 rounded-xl border border-border">
          <h2 className="font-display text-xl text-foreground mb-3">
            Suosituimmat tekstit {dialect.name}lla
          </h2>
          <ul className="space-y-2">
            {dialect.shirtTexts.map((text, i) => (
        ...
        <article className="prose prose-invert max-w-3xl mt-12 space-y-6 text-muted-foreground">
          <section>
            <h2 className="font-display text-2xl text-foreground mb-3">
              Murrelahja — miksi se toimii?
            </h2>
            <p>
              Murre on osa identiteettiä — se kertoo mistä tulet, kuka olet. Hauska lahja jossa on
              murreilmaisu tai alueellinen vitsi puhuttelee syvällä tasolla. Se ei ole vain lahja,
              se on tunnustus kotiseudulle.
            </p>
          </section>
          <section>
            <h2 className="font-display text-2xl text-foreground mb-3">
              Luonteenpiirteet: {dialect.characterTraits.join(", ")}
            </h2>
            <p>
              {dialect.name}puhujilla on oma erityinen identiteetti. Lahja joka tunnistaa nämä
              piirteet on aina osuva ja muistettava.
            </p>
          </section>
          <section>
            <h2 className="font-display text-2xl text-foreground mb-3">Usein kysytyt kysymykset</h2>
            <dl className="space-y-4">
              {dialect.faqs.map((f, i) => (
                <div key={i}>
        ...
        {/* Related dialects */}
        {relatedDialects.length > 0 && (
          <nav className="mt-12 pt-6 border-t border-border" aria-label="Muut murteet">
            <h2 className="font-display text-xl text-foreground mb-4">Muut murrelahjat</h2>
            <div className="flex flex-wrap gap-2">
              {relatedDialects.map((d) => (
                <Link
                  key={d.slug}
                  to={`/murre/${d.slug}`}
        ...
        {/* Browse all dialects */}
        <nav className="mt-8 pt-6 border-t border-border" aria-label="Kaikki murteet">
          <h2 className="font-display text-xl text-foreground mb-4">Selaa murrelahjoja</h2>
          <div className="flex flex-wrap gap-2">
            {dialects
              .filter((d) => d.slug !== dialect.slug)
              .map((d) => (
                <Link
                  key={d.slug}
                  to={`/murre/${d.slug}`}
